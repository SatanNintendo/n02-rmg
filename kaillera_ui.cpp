...
static KailleraColumnMenuItem g_users_column_menu[] = {
        { MENU_ID_USERS_COL_BASE + 0, "", 80 },
        { MENU_ID_USERS_COL_BASE + 1, "", 35 },
        { MENU_ID_USERS_COL_BASE + 2, "", 44 },
        { MENU_ID_USERS_COL_BASE + 3, "", 85 },
        { MENU_ID_USERS_COL_BASE + 4, "", 55 },
};

static KailleraColumnMenuItem g_lobby_column_menu[] = {
        { MENU_ID_LOBBY_COL_BASE + 0, "", 100 },
        { MENU_ID_LOBBY_COL_BASE + 1, "", 60 },
        { MENU_ID_LOBBY_COL_BASE + 2, "", 60 },
        { MENU_ID_LOBBY_COL_BASE + 3, "", 120 },
};

static KailleraColumnMenuItem g_games_column_menu[] = {
        { MENU_ID_GAMES_COL_BASE + 0, "", 285 },
        { MENU_ID_GAMES_COL_BASE + 1, "", 60 },
        { MENU_ID_GAMES_COL_BASE + 2, "", 130 },
        { MENU_ID_GAMES_COL_BASE + 3, "", 150 },
        { MENU_ID_GAMES_COL_BASE + 4, "", 50 },
        { MENU_ID_GAMES_COL_BASE + 5, "", 45 },
};
...
void kaillera_sdlgGameMode(bool toggle = false){
        ...
        if (!toggle){
                SetWindowText(GetDlgItem(kaillera_sdlg, IDC_CREATE), LNG(GAME_SWAP));
...
void kaillera_sdlgNormalMode(bool toggle = false){
        if (!toggle){
                ...
                hosting = false;
                SetWindowText(GetDlgItem(kaillera_sdlg, IDC_CREATE), LNG(GAME_CREATE));
        } else {
...
static int kaillera_game_status_rank(const char *status){
        if (status == NULL)
                return INT_MAX;
        if (strcmp(status, GAME_STATUS[0]) == 0)
                return 0;
        if (strcmp(status, GAME_STATUS[1]) == 0)
                return 1;
        return 2;
}
...
void kaillera_chat_callback(char*name, char * msg){
        if (MINGUIUPDATE && KSSDFA.state==2)
                return;
        kaillera_outpf(LNG(CHAT_MSG_FORMAT), name, msg);
}
void kaillera_game_chat_callback(char*name, char * msg){
        if (name != NULL && _stricmp(name, "server") == 0) {
                kaillera_gdebug_color(KAILLERA_COLOR_GREEN, LNG(CHAT_MSG_FORMAT), name, msg);
        } else {
                kaillera_gdebug(LNG(CHAT_MSG_FORMAT), name, msg);
        }
...
void kaillera_motd_callback(char*name, char * msg){
        ...
        if (is_pm) {
                ...
                char line[4096];
                char pm_text[2048];
                _snprintf_s(pm_text, sizeof(pm_text), _TRUNCATE, LNG(CHAT_PM_FORMAT), msg);
                _snprintf_s(line, sizeof(line), _TRUNCATE, "%s%s\r\n", ts, pm_text);
                re_append(kaillera_sdlg_partchat, line, KAILLERA_COLOR_GREEN);
        } else {
                kaillera_ui_motd(LNG(CHAT_MOTD_FORMAT), msg);
        }
}
void kaillera_user_join_callback(char*name, int ping, unsigned short id, char conn){
        ...
        kaillera_ui_debug(LNG(CHAT_JOIN_FORMAT), name, ping);
        kaillera_sdlg_userslvReSort();
}
void kaillera_user_leave_callback(char*name, char*quitmsg, unsigned short id){
        ...
        kaillera_ui_debug(LNG(CHAT_PART_FORMAT), name);
}
...
void kaillera_player_add_callback(char *name, int ping, unsigned short id, char conn){
        ...
        wsprintf(bfx, LNG(DELAY_FORMAT), thrp * conn - 1);
...
void kaillera_player_joined_callback(char * username, int ping, unsigned short uid, char connset){
        kaillera_ui_gdebug_color(KAILLERA_COLOR_DARK_BLUE, LNG(CHAT_PLAYER_JOIN_FORMAT), username);
...
void kaillera_player_left_callback(char * user, unsigned short id){
        kaillera_ui_gdebug_color(KAILLERA_COLOR_DARK_BLUE, LNG(CHAT_PLAYER_PART_FORMAT), user);
...
void kaillera_user_kicked_callback(){
        ...
        SwitchJoinMessageContext(false);
        hosting = false;
        kaillera_error_callback(LNG(CHAT_KICKED));
        KSSDFA.input = KSSDFA_END_GAME;
        KSSDFA.state = 0;
        kaillera_sdlgNormalMode();
}
void kaillera_login_stat_callback(char*lsmsg){
        kaillera_core_debug(LNG(CHAT_LOGIN_STATUS_FORMAT), lsmsg);
}
void kaillera_player_dropped_callback(char * user, int gdpl){
        kaillera_gdebug(LNG(CHAT_DROPPED_FORMAT), user, gdpl);
...
void kaillera_game_callback(char * game, char player, char players){
        ...
        playerno = player;
        numplayers = players;
        kaillera_gdebug(LNG(CHAT_STARTING_FORMAT), GAME, playerno, numplayers);
        kaillera_gdebug(LNG(CHAT_STARTING_HINT));

        if (MINGUIUPDATE)
                kaillera_gdebug(LNG(CHAT_MINIMAL_GUI_NOTICE));

        if (kaillera_RecordingEnabled())
                kaillera_gdebug(LNG(CHAT_RECORDING_NOTICE));
...
void kaillera_game_netsync_wait_callback(int tx){
        SetWindowText(kaillera_sdlg_ST_SPEED, LNG(CHAT_NETSYNC_WAIT));
        int secs = tx / 1000;
...
static bool IsNonGameLobbyName(const char* name) {
        if (name == NULL || *name == 0)
                return false;
        return strcmp(name, s_lobby_away) == 0 || strcmp(name, s_lobby_chat) == 0;
}
...
void kaillera_sdlg_create_games_list_menu() {
        ...
        if (kaillera_sdlg_GamesCount > 20) {
                ...
                AppendMenu(starMenu, MF_STRING, MENU_ID_CREATE_AWAY, s_lobby_away);
                AppendMenu(starMenu, MF_STRING, MENU_ID_CREATE_CHAT, s_lobby_chat);
        } else {
                if (kaillera_sdlg_GamesCount > 0)
                        AppendMenu(kaillera_sdlg_CreateGamesMenu, MF_SEPARATOR, 0, NULL);
                AppendMenu(kaillera_sdlg_CreateGamesMenu, MF_STRING, MENU_ID_CREATE_AWAY, s_lobby_away);
                AppendMenu(kaillera_sdlg_CreateGamesMenu, MF_STRING, MENU_ID_CREATE_CHAT, s_lobby_chat);
        }
}
...
void kailelra_sdlg_join_selected_game(){
        ...
        if (sel >= 0 && sel < kaillera_sdlg_gameslv.RowsCount() && !inGame) {
                ...
                if (strcmp(temp, GAME_STATUS[0]) != 0) {
                        kaillera_error_callback(LNG(ERR_JOIN_RUNNING_GAME));
                        return;
                }
                ...
                while (*cx != 0) {
                                if (strcmp(cx, temp) == 0) {
                                        ...
                                        if (MessageBox(kaillera_sdlg, LNG(ERR_EMU_MISMATCH_BODY), LNG(ERR_EMU_MISMATCH_TITLE), MB_YESNO | MB_ICONEXCLAMATION) != IDYES)
                                                return;
                                }
                                kaillera_join_game(id);
                                return;
                ...
                kaillera_error_callback(LNG(ERR_ROM_NOT_FOUND), temp);
        }
}
...
void kaillera_sdlg_show_games_list_menu(HWND handle, bool incjoin = false){
        ...
        if(result != 0){
                if(result == MENU_ID_JOIN){
                        ...
                        if (!inGame) {
                                const char* lobby = (result == MENU_ID_CREATE_AWAY) ? s_lobby_away : s_lobby_chat;
...
void kaillera_sdlg_show_users_list_menu(HWND hDlg) {
        ...
        HMENU menu = CreatePopupMenu();
        AppendMenu(menu, MF_STRING, MENU_ID_SEND_MSG, LNG(CTX_SEND_MESSAGE));
        AppendMenu(menu, MF_STRING, MENU_ID_FINDUSER, LNG(CTX_FIND_USER));
        AppendMenu(menu, MF_STRING, MENU_ID_IGNORE, LNG(CTX_IGNORE));
        AppendMenu(menu, MF_STRING, MENU_ID_UNIGNORE, LNG(CTX_UNIGNORE));
...
static void LoadOptions(HWND hwnd){
        ...
        SendMessage(flashCombo, CB_ADDSTRING, 0, (LPARAM)LNG(OPT_FALSE));
        SendMessage(flashCombo, CB_ADDSTRING, 0, (LPARAM)LNG(OPT_TRUE));
        SendMessage(flashCombo, CB_SETCURSEL, (flash == 1) ? 1 : 0, 0);

        HWND beepCombo = GetDlgItem(hwnd, IDC_BEEP);
        SendMessage(beepCombo, CB_ADDSTRING, 0, (LPARAM)LNG(OPT_FALSE));
        SendMessage(beepCombo, CB_ADDSTRING, 0, (LPARAM)LNG(OPT_TRUE));
        ...
        case WM_INITDIALOG:
                {
                        ...
                        {
                                char xx[256];
                                wsprintf(xx, LNG(TITLE_CONNECTING), kaillera_sdlg_NAME);
                        ...
                        kaillera_sdlg_userslv.AddColumn(LNG(COL_STATUS), 85);
                        kaillera_sdlg_userslv.AddColumn(LNG(COL_CONNECTION), 0);
                        ...
                        kaillera_sdlg_gameslv.AddColumn(LNG(COL_USER), 150);
                        kaillera_sdlg_gameslv.AddColumn(LNG(COL_STATUS), 50);
                        kaillera_sdlg_gameslv.AddColumn(LNG(COL_USERS), 45);
                        ...
                        kaillera_sdlg_LV_GULIST.AddColumn(LNG(COL_NICK), 100);
                        kaillera_sdlg_LV_GULIST.AddColumn(LNG(COL_PING), 60);
        ...
        case WM_TIMER:
                {
                        if (kaillera_sdlg_MODE==1) {
                                ...
                                if (kaillera_is_connected())
                                        wsprintf(xx, LNG(TITLE_CONNECTED), kaillera_sdlg_NAME, kaillera_sdlg_userslv.RowsCount(), kaillera_sdlg_gameslv.RowsCount());
                                else
                                        wsprintf(xx, LNG(TITLE_CONNECTING), kaillera_sdlg_NAME);
                                SetWindowText(kaillera_sdlg, xx);
                        } else {
                                char xx[256];
                                wsprintf(xx, LNG(TITLE_IN_GAME), GAME);
                                SetWindowText(kaillera_sdlg, xx);

                                int jf;
                                if ((jf = kaillera_get_frames_count()) != kaillera_sdlg_frameno) {
                                        char xxx[32];
                                        wsprintf(xxx, LNG(SPEED_FORMAT), jf - kaillera_sdlg_frameno, SOCK_SEND_PACKETS - kaillera_sdlg_pps);
                                ...
                                if ((jf = kaillera_get_delay()) != kaillera_sdlg_delay) {
                                        char xxx[32];
                                        wsprintf(xxx, LNG(DELAY_FORMAT), kaillera_sdlg_delay = jf);
                                        SetWindowText(kaillera_sdlg_ST_DELAY, xxx);
                                }
                        }
                }
                ...
                break;
                        ...
                        case BTN_START:
                                if (IsNonGameLobbyName(GAME)) {
                                        kaillera_error_callback(LNG(ERR_NO_GAME_IN_LOBBY));
                                } else {
                                        kaillera_start_game();
                                }
                                break;
                        ...
                        case BTN_ADVERTISE:
                                {
                                        ...
                                        if (hosting) {
                                                wsprintf(ad, LNG(ADV_HOST_FORMAT), GAME, maxplayers);
                                        } else {
                                                wsprintf(ad, LNG(ADV_JOIN_FORMAT), hostname, GAME, maxplayers);
                                        }
...
void ConnectToServer(char * ip, int port, HWND pDlg,char * name) {
        ...
        const char conset = 1; // Always treat as LAN (highest packet rate)
                ...
                char quitmsg[128];
                wsprintf(quitmsg, "%s" KAILLERA_VERSION, LNG(QUIT_MSG));
                ...
                MessageBox(pDlg, LNG(ERR_CORE_INIT_FAILED), 0, 0);
        }
}
//===============================================================================
//===============================================================================
