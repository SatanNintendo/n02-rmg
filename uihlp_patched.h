...
inline void get_timestamp(char* buffer, size_t size) {
        ...
        if (hour == 0) hour = 12;
        const char* ampm = (st.wHour >= 12) ? LNG(TIMESTAMP_PM) : LNG(TIMESTAMP_AM);
        if (size == 0)
                return;
        _snprintf_s(buffer, size, _TRUNCATE, LNG(TIMESTAMP_FORMAT), hour, st.wMinute, ampm);
}
