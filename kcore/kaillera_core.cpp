...
void kaillera_set_spoof_ping(int spoof_ping_ms) {
	if (spoof_ping_ms > 0 && spoof_ping_ms < 1000) {
		...
		kaillera_core_debug(LNG(K_PING_SPOOF_SET), spoof_ping_ms);
	} else {
...
bool kaillera_core_connect(char * ip, int port){
	...
	kaillera_core_debug(LNG(K_CONNECTING_TO), ip, port);
	...
	if (ksock.set_address(ip, port)){
		...
		if (ksock.has_data()) {
			...
			kaillera_core_debug(LNG(K_SERVER_REPLIED));
				...
				if (ksock.check_recv(srsp, &srspl, false, &addr)) {
					...
					if (srspl >= 9 && strncmp("HELLOD00D", srsp, 9) == 0) {
						...
						kaillera_core_debug(LNG(K_LOGGING_IN));
						
						int server_port = atoi(srsp + 9);
						if (server_port <= 0 || server_port > 65535) {
							kaillera_error_callback(LNG(K_ERR_CONNECTING));
							return false;
						}
					...
					if (kaillera_spoofing && kaillera_spoof_ping > 0) {
						kaillera_core_debug(LNG(K_PING_SPOOF_ENABLED), kaillera_spoof_ping);
					...
					if (strcmp("TOO", srsp) == 0) {
						kaillera_error_callback(LNG(K_SERVER_FULL));
					} else {
						kaillera_error_callback(LNG(K_ERR_CONNECTING));
	...
	case GAMEBEGN:
		{
			if (KAILLERAC.leave_game_requested) {
				kaillera_core_debug(LNG(K_IGNORING_GAMEBEGN));
				break;
			}
			//KAILLERAC.USERSTAT = 3;
			KAILLERAC.PLAYERSTAT = 1;
			...
			if (kaillera_30fps_mode && KAILLERAC.dframeno > 1) {
				...
				kaillera_core_debug(LNG(K_30FPS_HALVED), original, KAILLERAC.dframeno);
			}

			kaillera_core_debug(LNG(K_SERVER_DELAY_INFO),
...
void kaillera_step(){
	...
	if (KAILLERAC.pending_spoof_announce && p2p_GetTime() >= KAILLERAC.spoof_announce_time) {
		...
		char spoof_msg[64];
		sprintf(spoof_msg, LNG(K_PING_SPOOF_MSG), kaillera_spoof_ping, target_delay);
		...
		if (k_socket::check_sockets(0,100) && KAILLERAC.connection && KAILLERAC.connection->has_data()){
			...
			if (KAILLERAC.connection->receive_instruction(&ki, false, &saddr)){
				if (ki.type== GAMRSRDY) {
					...
					kaillera_core_debug(LNG(K_ALL_PLAYERS_READY));
					break;
				} else {
					kaillera_ProcessGeneralInstruction(&ki);
				}
				...
				if (KAILLERAC.kaillera_gv_queue.length <= 0) {
					if (ttx == 0) {
						...
						if (tttx - ttx > 10) {
							...
							pix++;
							if (pix == 500){
								kaillera_core_debug(LNG(K_LOST_CONNECTION));
								kaillera_game_drop();
								return -1;
							}
						}
