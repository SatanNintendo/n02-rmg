...
bool p2p_disconnect(){
	...
	if (P2PCORE.CONNECTED) {
		if (P2PCORE.status > 1) {
			p2p_core_debug(LNG(P2P_CANT_QUIT));
			return false;
		} else {
...
void p2p_print_core_status(){
	p2p_core_debug( "P2PCORE{ \r\n"
		...
		, P2PCORE.HOST? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.PORT
		, P2PCORE.HOST? LNG(P2P_IRRELEVENT) : P2PCORE.IP
		, P2PCORE.APP
		, P2PCORE.GAME
		, P2PCORE.USERREADY? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.PEERREADY? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.USERNAME
		, P2PCORE.PEERNAME
		, P2PCORE.ping
		, P2PCORE.pingc
		, P2PCORE.CONNECTED? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.USERLOADED? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.PEERLOADED? LNG(P2P_TRUE):LNG(P2P_FALSE)
		, P2PCORE.status
		, P2PCORE.throughput
		, P2PCORE.frameno
		, P2PCORE.DATALEN
...
void p2p_set_ready(bool bx){
	...
	if (P2PCORE.USERREADY != bx) {
		...
		p2p_core_debug(LNG(P2P_READY_MARKED), bx? LNG(P2P_READY):LNG(P2P_NOT_READY));
	}
}
...
bool p2p_WaitForPeerToLoadOrDie(){
	...
	p2p_core_debug(LNG(P2P_MUFFIN_LOADED));
	...
	while (!P2PCORE.PEERLOADED && P2PCORE.status == 1){
		if (P2PCORE.connection->has_data() || (k_socket::check_sockets(1,0) && P2PCORE.connection->has_data())){
			...
			if (P2PCORE.connection->receive_instruction(&ki, false, &saddr)) {
				switch(ki.inst.type){
				case LOAD:
					p2p_core_debug(LNG(P2P_DONUT_LOADED));
...
bool p2p_SynChronizeClocksOrDie(){
	...
	if (P2PCORE.HOST) {
		...
		if (p2p_frame_delay_override > 0) {
			...
			p2p_core_debug(LNG(P2P_DELAY_OVERRIDE), calculated_delay, P2PCORE.throughput);
		} else {
		...
		if (p2p_30fps_mode && P2PCORE.throughput > 1) {
			...
			p2p_core_debug(LNG(P2P_30FPS_HALVED), original, P2PCORE.throughput);
		}
	...
	while (P2PCORE.connection && P2PCORE.connection->has_data()){
		...
		if (P2PCORE.connection->receive_instruction(&ki, false, &saddr)){
					...
					if (P2PCORE.HOST && !P2PCORE.CONNECTED){
						if (P2PCORE.ping == -1 && ki.inst.flags == LOGN_REQ) {
							...
							ki.load_sstring(P2PCORE.PEERNAME);
								...
								p2p_core_debug(LNG(P2P_CONN_REQUEST), P2PCORE.PEERNAME, peerapp_base);
								p2p_peer_info_callback(P2PCORE.PEERNAME, peerapp_base);

								if (strcmp(peerapp_base, localapp_base)!=0)
									p2p_send_chat(LNG(P2P_EMU_DIFF_ALERT));
						...
						if (P2PCORE.ping == 0 && ki.inst.flags == LOGN_RPOS) {
							...
							p2p_core_debug(LNG(P2P_PEER_RECONFIRMED));
							...
							{ char tmp[256]; wsprintf(tmp, LNG(P2P_USING_VERSION_HOST), P2P_VERSION); p2p_send_chat(tmp); }
							...
							if (P2PCORE.ping == 0 && ki.inst.flags == LOGN_RNEG) {
								P2PCORE.ping = -1;
								p2p_core_debug(LNG(P2P_PEER_DROPPED_NO_GAME));
							}
						} else {
							if (!P2PCORE.HOST && !P2PCORE.CONNECTED){
								if (P2PCORE.ping == -1 && ki.inst.flags == LOGN_RPOS) {
									...
									p2p_core_debug(LNG(P2P_PEER_REPLIED), P2PCORE.PEERNAME, P2PCORE.GAME);
									...
									{ char tmp[256]; wsprintf(tmp, LNG(P2P_USING_VERSION_CLIENT), P2P_VERSION); p2p_send_chat(tmp); }

									p2p_peer_joined_callback();
									
								}
								if (P2PCORE.ping == -1 && ki.inst.flags == LOGN_RNEG) {
									p2p_core_debug(LNG(P2P_PEER_DROPPED_DIFF_EMU));
									P2PCORE.ping = -1;
								}
							}
						}
			...
			case PREADY:
				...
				p2p_core_debug(LNG(P2P_PEER_READY_FORMAT), P2PCORE.PEERNAME,P2PCORE.PEERREADY? LNG(P2P_READY):LNG(P2P_NOT_READY));
				if (P2PCORE.USERREADY && P2PCORE.PEERREADY) {
					p2p_send_chat(LNG(P2P_BOTH_READY));
					p2p_instruction kx;
					kx.inst.type = LOAD;
			...
			case EXIT:
					...
					if (P2PCORE.status) {
						p2p_core_debug(LNG(P2P_REINIT_SERVER));
						delete P2PCORE.connection;
						P2PCORE.connection = 0;
						...
						if (!P2PCORE.connection->initialize(P2PCORE.PORT)){
							p2p_core_debug(LNG(P2P_SOCKET_INIT_ERROR));
						} else p2p_core_debug(LNG(P2P_DONE));
		...
		// the UI) to detect this and recover.
		{
			...
			if (P2PCORE.CONNECTED &&
				...
				p2p_core_debug(LNG(P2P_PING_TIMEOUT));
				p2p_peer_left_callback();
	...
	while (P2PCORE.connection->has_data()){
		...
		if (P2PCORE.connection->receive_instructionx(&ki)){
			if (ki.inst.type == DATA) {
				...
				p2p_core_debug(LNG(P2P_PEER_DROPPED));
				P2PCORE.status = 1;
				P2PCORE.USERREADY = false;
				P2PCORE.PEERREADY = false;
				// Entering lobby from gameplay: reset ping timeout baseline to "now"
...
int p2p_modify_play_values(void *values, int size){
	...
	if (P2PCORE.status > 1) {
			...
			while (P2PCORE.PEERDATA.pos < P2PCORE.DATALEN && P2PCORE.status > 1){
				if (P2PCORE.connection->has_data() || (k_socket::check_sockets(0,10) && P2PCORE.connection->has_data())){
					...
					if (p2p_GetTime() - TI > 20) {
						...
						TO++;
						if (TO == 320) {
							p2p_core_debug(LNG(P2P_DATA_TIMEOUT));
							p2p_drop_game();
							return -1;
						}
					}
		...
		if (p2p_WaitForPeerToLoadOrDie()){
			//TRACE
			p2p_core_debug(LNG(P2P_EVERYONE_LOADED));
		}
		//TRACE
		if (P2PCORE.status == 2 && p2p_CalculatePingOrDie()){
			p2p_core_debug(LNG(P2P_CALCULATED_PING), P2PCORE.ping);
		}
		//TRACE
		if (P2PCORE.status == 2 && p2p_SynChronizeClocksOrDie()) {
			p2p_core_debug(LNG(P2P_CALCULATED_DELAY), P2PCORE.throughput);
		}
		//TRACE
		
		//kprintf(__FILE__ ":%i, %i", __LINE__, p2p_GetTime());
		p2p_core_debug(LNG(P2P_GAMESYNC_COUNTDOWN), (P2P_GAMESYNC_WAIT/1000)+1);
